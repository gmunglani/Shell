...
// assert a condition
#define ASSERT(cond) if(!(cond)) { *os << INFO << "ERROR: Condition " #cond " not fulfilled!" << std::endl; exit(1); }
#define REQUEST(cond) if(!(cond)) { *os << INFO << "WARNING: Condition " #cond " not fulfilled!" << std::endl; }
...
#define BASE_PARSE_CFG(cfg,type,var,name,def,def_typed) \
  if (!cfg.have_variable(name) && !command_line.search("-" name))\
    *os << INFO << "WARNING: Parameter " name " is missing! Using default: " << def << std::endl;\
...
#define MAKE_PARSE(type,var,def) MAKE_PARSE_CFG(config_file,type,var,def)
#define PARSE(var) BASE_PARSE_CFG(config_file,{},var,#var,var,var)
...
#define MAKE_VECTOR_PARSE(var,def) MAKE_VECTOR_PARSE_CFG(config_file,var,def)

// parse a string variable and turn it to uppercase
#define STR_PARSE(var,def) \
  MAKE_NAME_PARSE_CFG(config_file, std::string, var##_name, #var, #def); \
  std::transform(var##_name.begin(), var##_name.end(), var##_name.begin(), ::toupper);

// read a function from the config file and command line, register some variables, and store it in the functions map
#define MAKE_FUNCTION(var,def) \
  MAKE_NAME_PARSE_CFG(config_file, std::string, var##_expr, #var, #def);\
...
#define EVAL_BARE_FUNCTION(var,def) EVAL_BARE_FUNCTION_CFG(config_file,var,def)
...
// debugging helpers
#define HERE { *os << INFO << "Here!" << std::endl; }
#define EXIT { *os << INFO << "Exiting!" << std::endl; exit(0); }
#define PRINT(var) { *os << INFO << #var " = " << var << std::endl; }
